  ...
  return (
    <div>
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <ViewToggle scope="companies" view={view} locale={locale} labels={{ table: t.tableView, cards: t.cardView, error: t.loadError }} />
      </div>
      ...
      {result.error || profilesResult.error ? <ListQueryError locale={locale} retryHref={retryHref} /> : (
        <>
          ...
          {result.rows.length ? (
            view === "table" ? <CompaniesTable rows={result.rows} locale={locale} /> : <CompaniesCards rows={result.rows} locale={locale} />
