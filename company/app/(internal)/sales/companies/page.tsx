  ...
  return (
    ...
    <div>
      ...
      <form className="mt-6 grid gap-3 rounded-2xl border border-[#dce3eb] bg-white p-4 md:grid-cols-3 xl:grid-cols-9">
        ...
        <Select name="readiness" label={t.contactReadiness} value={params.readiness} options={[["all", `${t.contactReadiness}: ${t.all}`], ...Object.entries(contactReadinessLabels[locale])]} />
        <Select name="research" label={t.researchStatus} value={params.research} options={[["all", `${t.researchStatus}: ${t.all}`], ...Object.entries(researchStatusLabels[locale])]} />
        ...
        <button className="rounded-lg bg-[#17233a] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#25334b] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1d5cff] focus-visible:ring-offset-2 active:scale-[0.98] md:col-span-3 xl:col-span-2">{t.filter}</button>
      </form>
  ...
  return (
    <div className="mt-3 overflow-x-auto rounded-2xl border border-[#dce3eb] bg-white">
      <table className="w-full min-w-[1420px] text-left text-sm">
        ...
        <thead className="border-b border-[#dce3eb] bg-[#f8fafc] text-xs text-[#667286]"><tr><th scope="col" className="px-4 py-3">{t.company}</th><th scope="col" className="px-4 py-3">{t.score}</th><th scope="col" className="px-4 py-3">{t.activeJobCount}</th><th scope="col" className="px-4 py-3">{t.stage}</th><th scope="col" className="px-4 py-3">{t.contactReadiness}</th><th scope="col" className="px-4 py-3">{t.officialAddress}</th><th scope="col" className="px-4 py-3">{t.researchStatus}</th><th scope="col" className="px-4 py-3">{t.owner}</th><th scope="col" className="px-4 py-3">{t.nextAction}</th></tr></thead>
        <tbody className="divide-y divide-[#e8edf2]">
          {rows.map((row) => (
            <tr key={row.lead_id} className="transition hover:bg-[#f8fbff]">
              <td className="px-4 py-4"><Link href={`/sales/companies/${row.organization_id}`} className="font-semibold hover:text-[#1d5cff] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1d5cff]">{row.organization_name}</Link>{row.official_name && row.official_name !== row.organization_name ? <p className="mt-1 max-w-64 truncate text-xs text-[#5f6c80]">{row.official_name}</p> : null}<p className="mt-1 text-xs text-[#8792a2]">{formatOrganizationType(row.organization_type, locale)}</p></td>
              ...
              <td className="px-4 py-4"><ReadinessBadge value={row.contact_readiness} locale={locale} /></td>
              <td className="max-w-80 px-4 py-4 text-xs leading-5">{row.official_address ? safeHttpUrl(row.official_address_source_url) ? <a href={safeHttpUrl(row.official_address_source_url) ?? undefined} target="_blank" rel="noreferrer" className="hover:text-[#1d5cff] hover:underline">{row.official_address}</a> : row.official_address : "—"}</td>
              <td className="px-4 py-4"><ResearchStatus status={row.last_enrichment_status} at={row.last_enriched_at} locale={locale} /></td>
    ...
    <div className="mt-3 grid gap-4 xl:grid-cols-2">
      {rows.map((row) => (
        <article key={row.lead_id} className="rounded-2xl bg-white p-5 shadow-sm shadow-[#17233a]/7 ring-1 ring-[#dfe5ec] transition duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-[#17233a]/8">
          ...
          {row.official_address ? <p className="mt-4 rounded-lg bg-[#f4f7fa] px-3 py-2 text-xs leading-5 text-[#566378]">{row.official_address}</p> : null}
          <dl className="mt-5 grid grid-cols-2 gap-x-4 gap-y-4 border-t border-[#e8edf2] pt-4 text-xs sm:grid-cols-3">
            ...
            <div><dt className="text-[#8792a2]">{t.contactReadiness}</dt><dd className="mt-1"><ReadinessBadge value={row.contact_readiness} locale={locale} /></dd></div>
            <div><dt className="text-[#8792a2]">{t.researchStatus}</dt><dd className="mt-1"><ResearchStatus status={row.last_enrichment_status} at={row.last_enriched_at} locale={locale} /></dd></div>
...
function companyHrefParams(params: CompanyListParams) {
  return { q: params.q, stage: params.stage, grade: params.grade, readiness: params.readiness, research: params.research, owner: params.owner };
}

function ReadinessBadge({ value, locale }: { value: string; locale: "ja" | "ko" }) {
...
function ResearchStatus({ status, at, locale }: { status: string | null; at: string | null; locale: "ja" | "ko" }) {
  if (!status) return <span className="text-[#8792a2]">—</span>;
  return <div className="space-y-1"><StatusBadge status={status} label={localizedLabel(researchStatusLabels, status, locale)} /><p className="whitespace-nowrap text-xs text-[#8792a2]">{formatSalesDate(at, locale)}</p></div>;
}
