...
export const PRIVACY_COPY: Record<Locale, PrivacyCopy> = {
  ko: {
    title: "개인정보·분석 도구 안내",
    description: "GA4와 PostHog를 언제 사용하는지, 선택 내용을 어떻게 저장하는지, 어떤 비식별 이용 정보를 측정하는지 안내합니다.",
    ...
    storageTitle: "선택 저장",
    storageBody: "허용·거부 선택은 이 브라우저의 localStorage에 저장됩니다. 거부하면 GA4·PostHog 스크립트와 측정 요청을 보내지 않습니다. 브라우저 저장 정보를 지우면 다시 선택할 수 있습니다.",
    rightsTitle: "적용 범위",
    rightsBody: "이 사이트는 문의 양식 입력값이나 이메일 주소를 분석 도구에 보내지 않습니다. 채용 준비 안내 페이지는 정보 제공용이며, 분석 도구에서 개인정보 제출을 요구하지 않습니다.",
  ...
  ja: {
    ...
    description: "GA4とPostHogを利用する条件、同意内容の保存方法、サイトで測定する個人を特定できない利用データを説明します。",
    updated: "最終更新日：2026年7月17日",
    analyticsTitle: "Google Analytics 4 と PostHog",
    analyticsBody: "このサイトでは、利用者が明示的に許可した場合だけGA4とPostHogを読み込みます。ページパス、言語、コンテンツID、管轄、指定したクリック・閲覧イベント、Web Vitalsが測定対象です。許可すると、画面テキストと入力値をマスキングしたPostHogのセッションリプレイを記録します。ネットワークリクエストの本文とヘッダーは記録しません。自動クリック収集とヒートマップは使用せず、氏名、メールアドレス、電話番号など、個人を直接特定できる情報は分析イベントへ送信しません。",
    storageTitle: "選択の保存",
    storageBody: "許可・拒否の選択はブラウザのlocalStorageに保存します。拒否すると、GA4・PostHogのスクリプトと測定リクエストは読み込まれません。ブラウザの保存情報を削除すると、もう一度選択できます。",
    rightsTitle: "適用範囲",
    rightsBody: "このサイトは、問い合わせフォームの入力値やメールアドレスを分析ツールへ送信しません。採用準備案内は情報提供用で、分析ツールを通じて個人情報の入力を求めません。",
    bannerTitle: "アクセス解析を許可しますか",
    bannerBody: "許可すると、個人を特定できない利用イベント、Webパフォーマンス、画面テキストと入力値をマスキングしたセッションリプレイをGA4とPostHogで測定します。",
    accept: "許可する",
    decline: "拒否する",
    policy: "詳しく見る",
  },
